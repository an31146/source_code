...
// gcc.exe -Wall -O2 -pthread -std=c99 -o parallelMergeSort.exe parallelMergeSort.c
...
int main (int argc, char *argv[])
{
	...
	for(int i = 0; i < NUM; i ++)
	{
		lystbck[i] = rand()/(double)RAND_MAX;
	}
	...
	printf("Sequential mergesort took: %.3lf sec.\n", diff);
	...
	//Compute time difference.
	diff = end.tv_sec - start.tv_sec + (end.tv_usec-start.tv_usec) / 1000000.0;
	printf("Parallel mergesort took: %.3lf sec.\n", diff);
	...
	//Compute time difference.
	diff = end.tv_sec - start.tv_sec + (end.tv_usec-start.tv_usec) / 1000000.0;
	printf("Built-in qsort took: %.3lf sec.\n", diff);
