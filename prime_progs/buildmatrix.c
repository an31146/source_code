	  ...
	  for (int i = 0; i < nrels; i++)
		{
			...
			if (j == rels[i]->n)		// keep relation with "non-single" primes
			{
			...
			{
				printf("discarding relation for j = %d: ", INDEX(rels[i]->p[j]));
				output_relation_without_q(stderr, rels[i]);
			}
		}
      discard = nrels - k;
	...
	{ /* remove (excess-keep) heavier relations */
	  ...
	  for (int i = 0; i < nrels; i++)
	      ...
	      else if (w > minw) /* compare with current heavy relations */
		{
		  ...
		  for (int j = 0; weight (rels[remove[j]], fb) != minw; j++)
