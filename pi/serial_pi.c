...
#define ROUNDS 20000    	    /* number of times "darts" is iterated */
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < ROUNDS; i++) {
		...
		printf("   After %10lu throws, average value of pi = %12.10f\r",
			(DARTS * (i + 1)), avepi);
		}    
...
double dboard(int darts)
{
   ...
   unsigned int score,            /* number of darts that hit circle */
       n;
 ...
 ************************************************************************/
   ...
   for (n = 1; n <= darts; n++) {
      ...
      r = (double)(rand()<<15 | rand())/cconst;
      x_coord = (2.0 * r) - 1.0;
      r = (double)(rand()<<15 | rand())/cconst;
      y_coord = (2.0 * r) - 1.0;
      
      //printf("%.10f\t%.10f\t", (double)x_coord, (double)y_coord);
